...
-- Only short-lived request metadata is stored; never audio or transcripts.
...
create index pomodoist_voice_requests_period_idx on private.pomodoist_voice_requests (usage_period_id);
create index pomodoist_voice_requests_expiry_idx on private.pomodoist_voice_requests (expires_at);
alter table private.pomodoist_voice_requests enable row level security;
revoke all on private.pomodoist_voice_requests from public, anon, authenticated, service_role;

-- The Edge Function supplies the verified Auth user and a server-generated ID.
-- Lock the monthly counter before reading reservations so concurrent requests
-- cannot both reserve the last available unit. Completion retries are idempotent.
create function public.pomodoist_voice_quota(p_user_id uuid, p_request_id uuid, p_action text)
...
$$;
revoke all on function public.pomodoist_voice_quota(uuid,uuid,text) from public, anon, authenticated;
grant execute on function public.pomodoist_voice_quota(uuid,uuid,text) to service_role;

-- Keep the SDK signature, but writes always use the server's UTC month.
-- Voice usage can only be charged after the backend produces a transcript.
...
select cron.schedule('pomodoist-voice-request-prune', '*/10 * * * *',
  'delete from private.pomodoist_voice_requests where expires_at <= now()');
