...
UpdateOffer binaryOffer(
  ...
  tag: 'v2.0.0',
  version: UpdateVersion.parse('2.0.0'),
  notes: '',
  checksum: checksum,
  asset: UpdateAsset(
    ...
    url: Uri.parse(
      'https://github.com/Kabanya/Pomodoist/releases/download/v2.0.0/Pomodoist-x86_64.AppImage',
    ),
  ),
);
...
void main() {
  test(
    'release discovery reads subsequent pages and uses semantic precedence',
    () async {
      final pages = <int>[];
      ...
      final found = await source.findUpdate(
        current: UpdateVersion.parse('1.0.0'),
        target: fixtures.linuxTarget,
  ...
  group('verified downloads', () {
    ...
    setUp(() async {
      stage = await Directory.systemTemp.createTemp('pomodoist-download-test-');
    });
    tearDown(() async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          phases,
          containsAllInOrder([UpdatePhase.downloading, UpdatePhase.verifying]),
        );
    ...
    test(
      'corrupt digest is rejected and no executable or partial file remains',
      () async {
    ...
    for (final delta in [-1, 1]) {
      test(
        'incorrect byte count ($delta) leaves no installable artifact',
        () async {
          ...
          await expectLater(
            downloader.download(
              binaryOffer(bytes, size: bytes.length + delta),
              destination,
              (_, _) {},
            ),
            throwsA(isA<UpdateFailure>()),
    ...
    test('interrupted stream cleans up its partial file', () async {
      ...
      final downloader = UpdateDownloader(
        dio: client((_) => ResponseBody(interrupted(), 200)),
      );
      addTearDown(downloader.dispose);
      final destination = File('${stage.path}/release');
      await expectLater(
        downloader.download(binaryOffer(bytes), destination, (_, _) {}),
    ...
    test(
      'mismatched sidecar and API digest fail before fetching executable',
      () async {
        final checksum = utf8.encode(
          '${'b' * 64} *Pomodoist-x86_64.AppImage\n',
        );
        var requests = 0;
        final downloader = UpdateDownloader(
          dio: client((_) {
            requests++;
            return ResponseBody.fromBytes(checksum, 200);
          }),
        );
        addTearDown(downloader.dispose);
        final offer = binaryOffer(
          [1],
          digest: 'a' * 64,
          checksum: UpdateAsset(
            name: 'Pomodoist-x86_64.AppImage.sha256',
            size: checksum.length,
            url: Uri.parse(
              'https://github.com/Kabanya/Pomodoist/releases/download/v2.0.0/Pomodoist-x86_64.AppImage.sha256',
            ),
          ),
        );
        await expectLater(
          downloader.download(offer, File('${stage.path}/release'), (_, _) {}),
          throwsA(isA<UpdateFailure>()),
        );
        expect(requests, 1);
      },
