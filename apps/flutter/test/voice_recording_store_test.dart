...
void main() {
  ...
  test(
    'persists the original recording and locale across store instances',
    () async {
      ...
      final restoredStore = FileVoiceRecordingStore(
  ...
  test(
    'invalid metadata cannot reference or delete files outside the owned directory',
    () async {
      final outside = File('${temporary.path}/outside.wav');
      await outside.writeAsBytes(List.filled(100, 0));
      await store.createPath();
      final manifest = File(
        '${temporary.path}/pomodoist_voice_pending/recording.json',
      );
      await manifest.writeAsString(
        jsonEncode({'file': '../outside.wav', 'ownerId': 'user'}),
      );
  ...
  test('empty and missing recordings are not restored', () async {
    ...
    await manifest.writeAsString(
      jsonEncode({'file': File(path).uri.pathSegments.last, 'ownerId': 'user'}),
    );
    expect(await store.load(), isNull);
