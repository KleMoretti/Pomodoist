...
void main() {
  group('SemVer', () {
    ...
    test('build metadata is not precedence', () {
      expect(
        UpdateVersion.parse(
          'v1.2.3+94',
        ).compareTo(UpdateVersion.parse('1.2.3+95')),
        0,
      );
    });
    for (final value in [
  ...
  UpdateOffer? select(
    ...
    releases,
    current: UpdateVersion.parse(current),
    target: target,
    channel: channel,
  );
  ...
  test('sorts by version, not API publication order', () {
    expect(
      select([
        releaseFixture('v1.1.0'),
        releaseFixture('v2.0.0'),
        releaseFixture('v1.9.0'),
      ])?.tag,
      'v2.0.0',
    );
  });
  test('never downgrades RC or reinstalls metadata-only changes', () {
    ...
    expect(
      select([releaseFixture('v2.0.0')], current: '2.0.0-rc.1')?.tag,
      'v2.0.0',
    );
  });
  test('requires the exact architecture, never a substring match', () {
    expect(
      select([releaseFixture('v2.0.0', name: 'Pomodoist-aarch64.AppImage')]),
      isNull,
    );
    expect(
      select([releaseFixture('v2.0.0', name: 'Pomodoist-x86_64.AppImage.exe')]),
      isNull,
    );
    expect(
      select([
        releaseFixture('v2.0.0', name: 'Pomodoist-aarch64.AppImage'),
      ], target: const UpdateTarget(UpdateOS.linux, UpdateArch.arm64))?.tag,
      'v2.0.0',
    );
  });
  test('legacy Windows installer is x64 only', () {
    ...
    expect(
      select([
        release,
      ], target: const UpdateTarget(UpdateOS.windows, UpdateArch.x64)),
      isNotNull,
    );
    expect(
      select([
        release,
      ], target: const UpdateTarget(UpdateOS.windows, UpdateArch.arm64)),
      isNull,
    );
  });
  test('fails closed without integrity metadata', () {
    ...
    expect(
      select([
        releaseFixture('v2.0.0', checksum: false, digest: 'sha256:${'a' * 64}'),
      ]),
      isNotNull,
    );
    expect(
      select([releaseFixture('v2.0.0', checksum: false, digest: 'md5:abc')]),
      isNull,
    );
  });
  for (final url in [
  ...
  test('checksum is bound to the exact artifact filename', () {
    ...
    expect(
      parseUpdateChecksum(
        '$hash *Pomodoist-Setup.exe\r\n',
        'Pomodoist-Setup.exe',
      ),
      hash,
    );
    expect(
      () => parseUpdateChecksum('$hash other.exe', 'Pomodoist-Setup.exe'),
      throwsFormatException,
    );
    expect(
      () => parseUpdateChecksum(hash, 'Pomodoist-Setup.exe'),
      throwsFormatException,
    );
  });
}
