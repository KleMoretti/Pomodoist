...
void main() {
  ...
  test(
    'Apple cloud mode opens the existing microphone settings channel',
    () async {
      ...
      final controller = BackendVoiceController(
      ...
      expect(
        await controller.openSettings(VoiceSettingsDestination.microphone),
        isTrue,
      );
      expect(calls, [
        isA<MethodCall>()
  ...
  test(
    'successful transcript is emitted into the existing controller flow',
    () async {
      ...
      controller
          .start(const VoiceRecognitionConfig(locale: 'ru-RU'))
  ...
  test(
    'temporary failure preserves the original audio and locale for retry',
    () async {
      final store = MemoryStore();
      ...
      await expectLater(
        recognizer.stop(const VoiceRecognitionConfig(locale: 'fr-FR')),
  ...
  test(
    'abort suppresses late results without deleting recoverable audio',
    () async {
  ...
  test(
    'explicit cancellation discards audio and rejects late results',
    () async {
      final response = Completer<Object?>();
      final store = MemoryStore();
      final recognizer = BackendVoiceRecognizer(
  ...
  test(
    'a new controller can restore a recording but another account cannot',
    () async {
      final store = MemoryStore()
        ..pending = const VoiceRecording(
          path: '/recording.wav',
          ownerId: 'user',
          locale: 'ru-RU',
        );
  ...
  test(
    'denied microphone and signed-out state never start recording',
    () async {
      ...
      await expectLater(
        recognizer.start(const VoiceRecognitionConfig()),
        throwsA(isA<VoiceRecognitionException>()),
      );
  ...
  test('empty provider text and timeout both keep retryable audio', () async {
    for (final response in [
      Future<Object?>.value({'ok': true, 'text': ' '}),
      ...
      await recognizer.start(const VoiceRecognitionConfig());
      await expectLater(
        recognizer.stop(const VoiceRecognitionConfig()),
        throwsA(isA<VoiceRecognitionException>()),
      );
      expect(recognizer.canRetryTranscription, isTrue);
    }
  ...
  test(
    'cancel during recorder finalization discards the stopped file',
    () async {
  ...
  test(
    'backend permissions do not require Apple Speech authorization',
    () async {
      final recorder = FakeRecorder()..permission = false;
      final controller = BackendVoiceController(
        BackendVoiceRecognizer(
      ...
      expect(await controller.checkAccess(), {
        'microphone': 'notDetermined',
        'speech': 'authorized',
      });
      expect(await controller.checkAccess(request: true), {
        'microphone': 'denied',
        'speech': 'authorized',
      });
      recorder.permission = true;
  ...
  test(
    'configured recording limit is enforced by the existing controller',
    () async {
      final recorder = FakeRecorder();
      final controller = VoiceRecognitionController(
        recordedRecognizer: BackendVoiceRecognizer(
          recorder: recorder,
          store: MemoryStore(),
          ownerId: () => 'user',
          invoke: (_) async => {'ok': true, 'text': 'Auto stopped'},
        ),
        platformSupport: const VoicePlatformSupport(
          supportsRecordedSystem: true,
        ),
...
class FakeRecorder implements VoiceRecorder {
  ...
  Future<String?> stop() async {
    stops++;
    return stopping == null ? path : await stopping!.future;
  }
