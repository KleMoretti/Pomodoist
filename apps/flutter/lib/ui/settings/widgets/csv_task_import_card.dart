...
import 'package:pomodoist/ui/settings/view_models/csv_task_import_view_model.dart';
...
class _CsvTaskImportCardState extends ConsumerState<CsvTaskImportCard> {
  late CsvTaskImportState _state;

  @override
  Widget build(BuildContext context) {
    _state = ref.watch(csvTaskImportViewModelProvider);
      ...
      control: Wrap(
        ...
        children: [
          ShadButton(
            ...
            enabled: !_state.busy,
            onPressed: _state.busy ? null : _selectFile,
            leading: _state.busy
                ? SizedBox.square(
                    dimension: 18,
  ...
  Future<void> _selectFile() async {
    final viewModel = ref.read(csvTaskImportViewModelProvider.notifier);
    if (!viewModel.beginSelection()) return;
    try {
      final file = await (widget.pickFile ?? _openCsvFile)();
      if (file == null || !mounted) return;
      final preview = (await viewModel.prepare(
        await file.readAsBytes(),
      )).getOrThrow();
      if (!mounted) return;
      final confirmed = await _showPreview(preview);
      if (!confirmed || !mounted) return;
      final result = (await viewModel.commit()).getOrThrow();
      if (!mounted) return;
