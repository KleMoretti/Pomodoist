...
import 'package:pomodoist/ui/tasks/view_models/search_view_model.dart';
...
class _SearchScreenState extends ConsumerState<SearchScreen> {
  ...
  SearchViewModel get _viewModel => ref.read(
    searchViewModelProvider((_identity, widget.initialQuery)).notifier,
  );
  ...
  void _clearFilters() => _viewModel.clearFilters();
  void _retry() => _viewModel.retry();

  @override
  Widget build(BuildContext context) {
    final l10n = context.l10n;
    final viewState = ref.watch(
      searchViewModelProvider((_identity, widget.initialQuery)),
    );
    final query = viewState.query;
    final projectId = viewState.projectId;
    final projects = viewState.projects;
    final items = viewState.items;
    final loading = viewState.loading;
    final hasError = viewState.hasError;
    final progressById = taskSubtaskProgressById(viewState.allTasks);
    final scope = (query, projectId, viewState.status);
                      ...
                      children: [
                        ConstrainedBox(
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (value) => _viewModel.setProject(value),
                          ),
                        ),
                        DropdownButton<TaskSearchStatus>(
                          ...
                          onChanged: (value) {
                            if (value != null) _viewModel.setStatus(value);
                          },
                        ),
                        ShadButton.ghost(
