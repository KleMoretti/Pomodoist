...
class UpdateVersion implements Comparable<UpdateVersion> {
  ...
  static UpdateVersion parse(String text) =>
      tryParse(text) ??
  ...
  static UpdateVersion? tryParse(String text) {
    ...
    return UpdateVersion._(text.replaceFirst(RegExp(r'^v'), ''), [
      for (var i = 1; i <= 3; i++) BigInt.parse(match.group(i)!),
    ], pre);
  }
  ...
  int compareTo(UpdateVersion other) {
    ...
    for (var i = 0; i < prerelease.length && i < other.prerelease.length; i++) {
      ...
      final leftNumber = RegExp(r'^\d+$').hasMatch(left)
          ? BigInt.parse(left)
          : null;
      final rightNumber = RegExp(r'^\d+$').hasMatch(right)
          ? BigInt.parse(right)
          : null;
...
class UpdateOffer {
  ...
  Uri get releaseUrl =>
      Uri.https('github.com', '/Kabanya/Pomodoist/releases/tag/$tag');
}
...
UpdateOffer? selectUpdate(
  ...
  for (final raw in releases) {
    if (raw is! Map || raw['draft'] != false || raw['tag_name'] is! String)
      continue;
    ...
    for (final name in target.artifactNames) {
      ...
      final checksums = assets
          .where((a) => a.name == '$name.sha256' && a.size <= 4096)
          .toList();
...
String parseUpdateChecksum(String text, String artifactName) {
  if (text.length > 4096)
    throw const FormatException('Checksum file is too large.');
  String? result;
  ...
  return result ??
      (throw const FormatException(
        'No SHA-256 for the exact update artifact.',
      ));
}
