...
class UpdateDownloader {
  ...
  Future<ResponseBody> _open(UpdateAsset asset, String tag) async {
    if (!isTrustedUpdateUrl(asset.url, tag, asset.name)) {
      throw const UpdateFailure(
        'The update URL is not a trusted GitHub release asset.',
      );
    }
    var url = asset.url;
      ...
      final response = await _dio.get<ResponseBody>(
        ...
        options: Options(
          ...
          validateStatus: (status) =>
              status == 200 || const [301, 302, 303, 307, 308].contains(status),
        ),
      );
      final body = response.data;
      if (body == null) {
        throw const UpdateFailure('Empty update download response.');
      }
      if (response.statusCode == 200) return body;
      await body.stream.listen(null).cancel();
      final location = response.headers.value('location');
      if (location == null) {
        throw const UpdateFailure('Missing update redirect location.');
      }
  ...
  Future<String> _checksum(UpdateOffer offer) async {
    ...
    if (sidecar != null) {
      ...
      await for (final chunk in response.stream.timeout(
        ...
        if (bytes.length > 4096) {
          throw const UpdateFailure('Checksum file is too large.');
        }
      }
      if (bytes.length != sidecar.size) {
        throw const UpdateFailure('Incomplete checksum download.');
      }
      final checksum = parseUpdateChecksum(
        utf8.decode(bytes),
        offer.asset.name,
      );
      if (expected != null && expected != checksum) {
        throw const UpdateFailure(
          'The release checksum and GitHub digest disagree.',
        );
      }
      expected = checksum;
    }
    if (expected == null || !RegExp(r'^[a-f0-9]{64}$').hasMatch(expected)) {
      throw const UpdateFailure(
        'The release does not provide a valid SHA-256 checksum.',
      );
    }
    return expected;
  }
    ...
    try {
      ...
      try {
        await sink.addStream(
          response.stream.timeout(const Duration(seconds: 30)).map((chunk) {
            ...
            if (received > offer.asset.size) {
              throw const UpdateFailure(
                'The update is larger than its declared size.',
              );
            }
            if (throttle.elapsedMilliseconds >= 100 ||
                received == offer.asset.size) {
        ...
        try {
          ...
          /* Preserve the source error. */
        }
        rethrow;
      }
      await sink.close();
      if (received != offer.asset.size) {
        throw const UpdateFailure('The update download was interrupted.');
      }
      progress(UpdatePhase.verifying, null);
      final actual = (await sha256.bind(partial.openRead()).first).toString();
      if (actual != expected) {
        throw const UpdateFailure(
          'Update integrity check failed. Nothing was installed.',
        );
      }
      ...
      throw const UpdateFailure(
        'Update download failed. Check your connection and retry.',
      );
    } finally {
      deadline.cancel();
