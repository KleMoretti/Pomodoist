...
class FileVoiceRecordingStore implements VoiceRecordingStore {
  ...
  Future<Directory> _storage() async {
    ...
    return _directory = await Directory(
      p.join(parent.path, 'pomodoist_voice_pending'),
  ...
  Future<VoiceRecording?> load() async {
    ...
    try {
      ...
      if (data is! Map ||
          data['file'] is! String ||
          !_fileName.hasMatch(data['file'] as String) ||
          data['ownerId'] is! String ||
          (data['locale'] != null && data['locale'] is! String)) {
        return null;
      }
  ...
  Future<void> save(VoiceRecording recording) async {
    ...
    if (!_fileName.hasMatch(name) ||
        !p.equals(file.parent.path, directory.path) ||
  ...
  Future<Uint8List> read(VoiceRecording recording) async {
    final directory = await _storage();
    final file = File(recording.path);
    if (!p.equals(file.parent.path, directory.path) ||
        !_fileName.hasMatch(p.basename(file.path))) {
      throw const FormatException('Invalid recording');
    }
